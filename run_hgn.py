...
def readfile(filename, type_=None):
    ...
    if len(sentence) > 0:
        ...
        sentence = []
        label = []
    # joblib.dump(data,'./data_all_o.pkl')
    return data
...
def readfile_label(train_file, test_file, dev_file):
    ...
    f_dev = open(dev_file, encoding="utf-8")
...
def main():
    ...
    # ------------------------------------------------------------------------------------------------------------------------
    parser.add_argument("--use_gate", default=True, help="门控机制")
    # ---任务1. False-------
    parser.add_argument("--use_attention", default=False, help="注意力机制")

    parser.add_argument(
        "--train_data_dir",
        default="data/Mu_gpt_BC5/50shot/mu_gpt_train-5.txt",
        type=str,
        # required=True,
        help="训练集路径",
    )
    # --------任务1----data/Mu_GPT_NCBI/5shot （5\20\50）/mu_gpt_train-1（1这里 1 - 5 ）  .txt  跑完1-5之后 把5shot 改为20shot 跑1-5  ，再改成50shot 再跑1-5
    parser.add_argument(
        "--dev_data_dir",
        default="data/Mu_gpt_BC5/50shot/mu_gpt_dev-5.txt",
        type=str,
        # required=True,
        help="验证集路径",
    )
    ...
    parser.add_argument(
        "--test_data_dir",
        default="data/Mu_gpt_BC5/mu_bc5_test.txt",
        type=str,
        help="测试集路径",
    )

    # -----------------------------------------------------------------------------------------------------------------------------
    parser.add_argument("--use_dconv", default=True, help="dconv model")
    ...
    parser.add_argument("--use_crf", action="store_true", help="Whether use crf")

    parser.add_argument(
        "--bert_model",
        default="BENT-PubMedBERT-NER-Disease",
        type=str,  # required=True,
        help="Bert pre-trained model selected in the list: bert-base-uncased, "
        "bert-large-uncased, bert-base-cased, bert-large-cased, bert-base-multilingual-uncased, "
        "bert-base-multilingual-cased, bert-base-chinese.  biobert-large-cased-v1.1  BENT-PubMedBERT-NER-Disease",
    )
    parser.add_argument(
        "--task_name",
        default="ner",
    ...
    parser.add_argument(
        "--do_predict", action="store_true", help="Whether to run eval or not."
    )
    ...
    parser.add_argument(
        "--seed", type=int, default=42, help="random seed for initialization"  # 42
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--hidden_dropout_prob", default=0.1, type=float, help="hidden_dropout_prob"
    )

    parser.add_argument("--window_size", default=-1, type=int, help="window_size")

    parser.add_argument(
        "--d_model", default=768, type=int, help="pre-trained model size"
    )
    ...
    parser.add_argument(
        "--windows_list", type=str, default="1qq3qq5qq7", help="window list"
    )
    parser.add_argument(
        "--connect_type", type=str, default="mlp-att", help="window list"
    )
    ...
    model = HGNER(
        ...
        windows_list=(
            [int(k) for k in args.windows_list.split("qq")]
    ...
    if args.do_train:
        ...
        all_seq_lens = torch.tensor(
            [f.seq_len for f in train_features], dtype=torch.long
        )
            ...
            for (
                ...
                if not args.use_crf:
                    try:
                        ...
                        print("shape error *******------", logits.shape)
